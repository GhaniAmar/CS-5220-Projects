...
def read_file(path):
    ...
    handle.close()
    lines = filter(lambda x : x[:10] != 'checkpoint', lines)
...
def make_plot(labels, outpath):
    ...
    title(texify('Floyd-Warshall Naive MPI Weak Scaling'), fontsize=18)
...
def strong_plot(outpath, x, y):
    ...
    title(texify('Floyd-Warshall Naive MPI Strong Scaling'), fontsize=18)
    grid(True)
    savefig(outpath, transparent=True)
