...
int square(int n,               // Number of nodes
           ...
           int rank,
           int size,
           int* restrict recvcounts,
           int* restrict displs,
           double* comp_time,
           double* comm_time)  
{
    int startindex= rank*floor(n/size);
    int endindex;
    double start = MPI_Wtime();

    if (rank!=(size-1))
        endindex= (rank+1)*floor(n/size);
    else
        endindex= n;
    ...
    MPI_Allgatherv(lnew+n*startindex, n*(endindex-startindex), 
                   MPI_INT, l, recvcounts, displs, 
...
void shortest_paths(int n, int* restrict l, int rank, int size, 
                    int* restrict recvcounts, int* restrict displs,
                    double* comp_time, double* comm_time)
{
    ...
    for (int done = 0; !done; )
      done = square(n, l, lnew, rank, size, recvcounts, displs, comp_time, comm_time);
...
{
    int n, c, rank, size, k;
    double p, t0, t1, comp_time, comm_time;
    int* l;
    ...
    int recvcountsarr[size];
    int displsarr[size];
    for (k= 0; k<size; k++) {
        displsarr[k]= n*k*floor(n/size);
        recvcountsarr[k]= n*floor(n/size);
    }
    recvcountsarr[size-1]= n*n-n*(size-1)*floor((n/size));
    int* restrict recvcounts= &recvcountsarr[0];
    int* restrict displs= &displsarr[0];
  
    t0 = MPI_Wtime(); 
  
    shortest_paths(n, l, rank, size, recvcounts, displs, &comp_time, &comm_time);
